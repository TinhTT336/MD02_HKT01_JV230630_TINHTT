...
export default function Card() {
    ...
    const [productList, setProductList] = useState(() => {
        const products = JSON.parse(JSON.stringify("productList")) || [];
        return products;
    });
    ...
    const addToCart = (id) => {
        ...
        // const list = productList.map(p = p.id === addProduct.id ? (p = { ...p, p.quantity + 1 }) : ())
    ...
    return (
        <>
            {showCart ? (<Cart handleClose={handleClose} addToCart={addToCart} quantity={quantity} productList={productList} />) : (<></>)}
